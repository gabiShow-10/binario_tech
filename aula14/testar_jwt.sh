...
curl -s -X POST http://localhost:3013/api/v1/auth/register \
  -H "Content-Type: application/json" \
  -d '{ "email": "operador@binariotech.com.br", "senha": "SenhaSegura123!", "perfil": "ADMIN" }' | jq .

echo -e "\n[2] Realizando Login e obtendo JWT..."
LOGIN_RESP=$(curl -s -X POST http://localhost:3013/api/v1/auth/login \
  -H "Content-Type: application/json" \
...
curl -s http://localhost:3013/api/v1/auth/perfil | jq .

echo -e "\n[4] Acessando Rota Protegida COM Token JWT Válido (Esperado HTTP 200)..."
curl -s http://localhost:3013/api/v1/auth/perfil \
